...
export const createBasicInformation = (req: IUserRequest, res: Response) => {
    const { id } = req.user
    const { organization_name, address, image, industry_type, summary, phone_number, email, cover_image }: IEmployerBasicInformation = req.body
    ...
    if (isValidBasicInformation(req.body)) {
        pool.query('select * from users where user_id = $1', [id], function (err: Error, result: QueryResult) {
            ...
            if (result.rowCount > 0) {
                ...
                pool.query(userQuery, [id, organization_name, phone_number], (err: Error, result: QueryResult) => {
                    ...
                    pool.query(query, [id, organization_name, industry_type, address, summary, phone_number, email], async function (err: Error, result: QueryResult) {
                        ...
                        if (image !== null && image?.url === undefined && typeof image === 'string') {
                            ...
                            if (!imageData) {
                                res.status(400).send({ message: 'Error on upload image' })
                            }
                        ...
                        pool.query(imageQuery, [id, image_url, image_public_id], async (err: Error, result: QueryResult) => {
                            if (err) return res.status(400).send({ message: 'Error on uploading image, image must be string' })
                            // cover images 
                            let coverImageUrl = cover_image?.url
                            let coverImagePublicId = cover_image?.public_id
                            if (cover_image !== null && coverImageUrl === undefined && typeof cover_image === 'string') {
                                const imageData = await uploadCoverImage(cover_image, id)
                                if (!imageData) {
                                    res.status(400).send({ message: 'Error on upload cover_image , cover_image must be string' })
                                }
                            ...
                            pool.query(coverImageQuery, [id, coverImageUrl, coverImagePublicId], (err: Error, result: QueryResult) => {
                                if (err) return res.status(400).send({ message: err })
                                return res.status(201).send({ message: 'Successfully created basic_information, images, cover_images, and updated users ' })
                            })
                        })
                    })
                })
...
export const updateBasicInformation = (req: IUserRequest, res: Response) => {
    const { id } = req.user
    const { organization_name, industry_type, address, image, cover_image, summary, phone_number }: IEmployerBasicInformation = req.body
        ...
        pool.query('select * from employers_basic_information where user_id = $1', [id], function (err: Error, result: QueryResult) {
            ...
            if (result.rowCount > 0) {
                ...
                pool.query(userQuery, [id, organization_name, phone_number], (err: Error, result: QueryResult) => {
                    ...
                    pool.query(query, [id, organization_name, industry_type, address, summary, phone_number], async function (err: Error, result: QueryResult) {
                        if (err) return res.status(400).send({ message: 'Failed to update basic_information' })
                        //    update image 
                        let public_id = image?.public_id
                        let url = image?.url
                        // if url is anavailable , upload to the cloudinary and get url
                        if (url === undefined && image !== null && typeof image === 'string') {
                            const imageData = await uploadImage(image, id)
                            if (!imageData) {
                        ...
                        pool.query('update images set url = $2 , public_id = $3 where user_id = $1', [id, url, public_id], async (err: Error, result: QueryResult) => {
                            if (err) return res.status(400).send({ message: 'Failed to update image' })
                            ...
                            const coverImageQuery = 'update cover_images set url = $2 , public_id = $3 where user_id = $1'
                            pool.query(coverImageQuery, [id, coverImageUrl, coverImagePublicId], (err: Error, result: QueryResult) => {
                                if (err) return res.status(400).send({ message: err })
                                return res.status(201).send({ message: 'Successfully created basic_information, images, cover_images, and updated users ' })
                            })
