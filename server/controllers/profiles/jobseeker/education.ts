...
export const createEducation= (req: IUserRequest, res: Response) => {
    ...
    if (marks !== undefined && marks !== null) {
        if (
            ...
            return res.status(400).send({ message: 'either remove or fill all the fields of marks object' });
        }
    
        if (Object.keys(marks).length > 0) {
            const invalidKeys = Object.keys(marks).filter(key => key !== 'value' && key !== 'type');
            ...
            if (isValidEducation(req.body)) {
                pool.query('select * from users where user_id = $1', [id], function (err: Error, result: QueryResult) {
                    ...
                    if (result.rowCount > 0) {
                        ...
                        pool.query(query, [id,course,degree, institute_name, graduation_year, marks,location ], function (err: Error, result: QueryResult) {
                            if (err) return res.status(400).send({ message: err })
                ...
                return res.status(400).send({
                    message: 'Invalid education format', format: `course,institute_name, graduation_year,degree, marks,location `});
            }
        } 
        ...
        export const updateEducation = (req: IUserRequest, res: Response) => {
            ...
            if (marks !== undefined && marks !== null) {
                ...
                if (Object.keys(marks).length > 0) {
                    ...
                    if (isValidEducation(req.body)) {
                        pool.query('select * from jobseekers_education where user_id = $1 and id = $2', [id, educationId], function (err: Error, result: QueryResult) {
                            if (err) return res.status(400).send({ message: err })
                            if (result.rowCount > 0) {
                                const query = `UPDATE jobseekers_education
                                SET course = $2,degree = $3, institute_name = $4, graduation_year = $5, marks = $6,location = $7
                                WHERE  id = $1 and user_id = $8`
                                pool.query(query, [educationId,course,degree, institute_name, graduation_year, marks,location,id ], function (err: Error, result: QueryResult) {
                                    if (err) return res.status(400).send({ message: err })
                                    return res.status(201).send({ message: 'Success' })
                                })
                            } else {
                                return res.status(400).send({ message: 'Education not found, make a post request for creating education' })
                            }
                        })
                    }
                    else {
