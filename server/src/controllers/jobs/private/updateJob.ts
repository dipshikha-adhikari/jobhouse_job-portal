...
export const updateJob = async (req: IUserRequest, res: Response) => {
  ...
  pool.query(userQuery, [id], function (err: Error, result: QueryResult) {
    ...
    if (role !== 'employer')
      return res.status(400).send({ message: 'Role should be employer' })
    ...
    pool.query(
      ...
      function (err: Error, result: QueryResult) {
        if (err)
          return res
            .status(400)
            .send({ error: 'Employer and job did not match' })
        if (result.rowCount > 0) {
          ...
          if (isValidJobData(req.body)) {
            ...
            return res.status(400).send({
              ...
              format:
                ' description, deadline, title, salary, category_id, level, location, type, no_of_vacancy, experience_required'
            })
          }
        } else {
