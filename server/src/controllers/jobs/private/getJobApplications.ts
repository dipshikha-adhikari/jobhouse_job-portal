...
export const getJobApplications = async (req: IUserRequest, res: Response) => {
  ...
  pool.query(
    'select * from jobs where job_id = $1 and employer_id = $2',
    [jobId, id],
    function (err: Error, result: QueryResult) {
      if (err) {
        return res.status(400).send({
          message: 'Can not find jobs for this employer_id'
        })
      }
      if (result.rowCount === 0) {
        return res
          .status(400)
          .send({ message: 'This job has not been created by you' })
      }
      pool.query(
        query,
        [jobId, id, limit, offset],
        function (err: Error, result: QueryResult) {
          if (err) {
            return res
              .status(400)
              .send({ message: 'Can not get job applications' })
          }
