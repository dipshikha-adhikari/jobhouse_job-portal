...
export const createOtherInformation = (req: IUserRequest, res: Response) => {
  ...
  if (isValidOtherInformation(req.body)) {
    pool.query(
      'SELECT * FROM employers_other_information WHERE user_id = $1',
      [id],
      function (err: Error, result: QueryResult) {
        if (err) return res.status(400).send({ message: err })
        if (result.rowCount > 0) {
          res.status(400).send({
            message: 'Already exists, please make put request'
          })
        } else {
          pool.query(
              ...
              if (result.rowCount > 0) {
                ...
                if (role !== 'employer') {
                  return res.status(401).send({
                    message: 'Only employer is allowed'
                  })
                }
                const query = `insert into employers_other_information (user_id, website ) values($1, $2)`
                pool.query(query, [id, website], function (err: Error) {
                  if (err) return res.status(400).send({ message: err })
    ...
    return res.status(400).send({
      message: 'Invalid other_information format',
      format: `website `
    })
  }
}
