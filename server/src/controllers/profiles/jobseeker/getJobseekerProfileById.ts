...
`
  try {
    const employer: QueryResult = await pool.query(
    ...
    if (employer.rowCount > 0) {
      ...
      const experience: QueryResult = await pool.query(experienceQuery, [
        jobseekerId
      ])
      profile['experience'] = experience.rows
      const education: QueryResult = await pool.query(educationQuery, [
        jobseekerId
      ])
