...
import { isValidBasicInformation } from '../../../helpers/validateJobseekerProfileInsertion'
...
export const createBasicInfo = (req: IUserRequest, res: Response) => {
  ...
  const {
    ...
    gender,
    image
  }: IJobseekerBasicInformation = req.body
  ...
  if (isValidBasicInformation(req.body)) {
          ...
          pool.query(existsQuery, [id], (err: Error, result: QueryResult) => {
            ...
            if (result.rowCount > 0) {
              return res.status(400).send({
                message: 'Basic information already exists'
              })
            }
          })

          const userQuery = `update users set fullname = $2 , phone_number = $3 where user_id = $1`
          pool.query(userQuery, [id, fullname, phone_number], (err: Error) => {
            if (err)
              return res.status(400).send({
            ...
            pool.query(
              ...
              async function (err: Error) {
                if (err)
                  return res.status(400).send({
                    message: 'Error on inserting jobseekers_basic_information'
                  })
                ...
                pool.query(imageQuery, [id, url, public_id], (err: Error) => {
                  if (err) return res.status(400).send({ message: err })
                  return res.status(201).send({
                    message:
                      'Successfully created basic_information, images and updated users '
                  })
                })
              }
            )
    ...
    return res.status(400).send({
      ...
      format: `fullname, date_of_birth, phone_number, permanent_address,current_address, gender `
    })
  }
}
...
export const updateBasicInfo = (req: IUserRequest, res: Response) => {
  ...
  if (isValidBasicInformation(req.body)) {
    pool.query(
      'select * from jobseekers_basic_information where user_id = $1',
      [id],
      function (err: Error, result: QueryResult) {
        if (err) return res.status(400).send({ message: err })
        if (result.rowCount > 0) {
                ...
                async (err: Error) => {
                  ...
                  pool.query(
                    ...
                    () => {
                      return res.status(201).send({
                        message:
                          'Successfully updated users_basic_information, users and images'
                      })
                    }
                  )
                }
          ...
          return res.status(400).send({
            message: 'Basic information not found, make a post request'
          })
        }
      }
    )
