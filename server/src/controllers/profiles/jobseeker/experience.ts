...
export const updateExperience = (req: IUserRequest, res: Response) => {
  ...
  if (isValidExperience(req.body)) {
    pool.query(
      'select * from jobseekers_experience where user_id = $1 and id = $2',
      [id, experienceId],
      function (err: Error, result: QueryResult) {
        if (err) return res.status(400).send({ message: err })
        if (result.rowCount > 0) {
          ...
          return res.status(400).send({
            message: 'Experience with this experienceId is not found'
          })
        }
      }
    )
...
export const deleteExperience = (req: IUserRequest, res: Response) => {
  ...
  const { experienceId } = req.params
  pool.query(
    'delete from jobseekers_experience where id = $1 and user_id = $2',
    [experienceId, id],
    (err: Error) => {
