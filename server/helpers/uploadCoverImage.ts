...
export const uploadCoverImage = async (image: any, user_id: string) => {
  try {
    const uploadOptions = {
      use_filename: true,
    ...
    const imageFromDb: QueryResult = await pool.query('select * from cover_images where user_id = $1', [user_id])
