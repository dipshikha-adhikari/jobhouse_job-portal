...
const Applications = () => {
  ...
  const {
    ...
    const result = await privateRequest.get(
      `api/v1/jobs/applications?jobId=${jobId}`,
    );
    return result.data;
  });
      ...
      <div className=" w-full grid gap-sm  ">
        <header className="text-center w-fit mx-auto bg-green-50 p-sm ">
          <h2 className="text-black-light">
            Below are the jobseekers that have applied for this job. You can
            view every jobseeker profile by clicking on the box
          </h2>
        </header>
