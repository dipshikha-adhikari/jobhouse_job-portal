...
import { MdOutlineMapsHomeWork, MdOutlineRoomPreferences } from "react-icons/md";
import { GiFlagObjective, GiSkills } from "react-icons/gi";
import { CiCalendar, CiCircleCheck, CiLocationOn, CiUser } from "react-icons/ci";
    ...
    <div className="sm:px-xl lg:border-sm lg:p-xl max-w-4xl w-full mx-auto">
      ...
      <div className="grid  gap-md  ">
          <section>
            ...
            <div className="grid gap-2 ">
              <div className="font-semibold flex gap-2"> 
                ...
                {profile?.basic_information?.gender === 'female' && <span>MRS</span>}
                {profile?.basic_information?.fullname || fullName}
              </div>
              <p>
                Current Address : {profile?.basic_information?.current_address || 'Not available'}
              </p>
              ...
              <p>
                Date of Birth :{" "}
                {profile?.basic_information?.date_of_birth ? moment(profile?.basic_information?.date_of_birth).format('YYYY-MM-DD')  :    'Not available'
          ...
          <section className="">
            ...
            <p className="py-md">
              {profile?.job_preference?.summary || (
            'Not available'
              )}
            </p>
          </section>
            ...
            {profile?.experience.length > 0 ? (
              <div className="grid gap-xs">
                {profile.experience.map((exp) => {
                  return (
                    <div className=" h-fit md:flex items-start grid gap-2 md:gap-md lg:gap-xl " key={exp.id}>
                      <div className="grid h-fit gap-2">
                      <p className="text-black-light italic flex items-center gap-xs">
                      <FaRegCalendarAlt />  {moment(exp.start_date).format("MMM Do YYYY")} -{" "}
                          {moment(exp.end_date).format("MMM Do YYYY")}
                        </p>
                        <p className="flex items-center gap-xs">
                        <CiUser />   <span className="font-semibold">{exp.job_title}</span>{" "}
                          ({exp.job_level})
                        </p>
                    ...
                    <div className="grid gap-2">
                        <p className="flex items-center gap-xs">
                        <MdOutlineMapsHomeWork />   {exp.organization_name} ({exp.organization_type})
                        </p>
                        <p className="flex items-center gap-xs"><CiLocationOn />{exp.job_location}</p>
                      
                      {exp.duties !== undefined &&   <div
          ...
          <section className="grid gap-xs">
            ...
            {profile?.job_preference ? (
              <div className="grid gap-2">
                <p className="flex items-center gap-2"> <CiCircleCheck /> Looking for : {profile?.job_preference?.job_level}</p>
                ...
                <p className="flex items-center gap-2">  <CiCircleCheck /> Available for : {profile.job_preference?.available_for}</p>
                <p className="flex items-center gap-2">
                <CiCircleCheck />   Expected Salary : (Equals) NRs{" "}
                  {profile.job_preference?.expected_salary}
                </p>
                <p className="flex items-center gap-2">
                <CiCircleCheck />   Preferred Location : {profile.job_preference.job_location}
                </p>
              </div>
            ) : (
             'Not available'
