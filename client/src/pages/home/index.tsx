...
const {data:companies , isLoading:loadingComapnies, isError:errorComapnies} = useQuery('topComapnies', getTopCompanies)
    ...
    <div className="grid gap-sm">
      ...
      <section className="grid gap-lg lg:flex">
        <aside className="grid gap-md flex-1 h-fit">
          ...
          <div className="grid gap-sm">
            <h2 className=" uppercase font-semibold text-green-dark text-xl border-y-md w-fit p-xs border-green-light flex items-center gap-2"> <MdHomeWork />Top Companies</h2>
...
return <Link  key={item.user_id}   to={`/employer/${item.organization_name}/${item.user_id}`} className="grid  text-black-dark hover:text-black-dark gap-1 border-xs  p-sm place-content-start">
  <img src={item.image_url} alt="image"  className="w-20 h-20"/>
  <p className="grid "><span className="font-semibold">{item.organization_name} </span>({item.industry_name})</p>
  <p className="text-green-dark font-semibold"> {item.job_count} jobs</p>
</Link>
})}
            </div>
          </div>
